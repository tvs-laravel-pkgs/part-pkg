app.component('variantList', {
    templateUrl: variant_list_template_url,
    controller: function(HelperService, $rootScope, $routeParams, $scope, $http, $timeout, $location) {
        ...
        var dataTable = $('#variant-table').DataTable({
            "dom": dom_structure_2,
            "language": {
                "search": "",
            ...
            responsive: true,
            ajax: {
                url: laravel_routes['getVariantList'],
                data: function(d) {
        ...
        $('.title-block').html('<h1 class="title">Variant<span class="badge badge-secondary" id="table_info">0</span></h1>');
        ...
        $('.page-header-content-right .button-block').html(
            '<a href="#!/part-pkg/variant/form" type="button" class="btn btn-primary">' +
            'Add New' +
            '</a>'
        );
        ...
        $('.search_clear').on("click", function() {
            $('#variant-table').DataTable().search('').draw();
        });
        $rootScope.loading = false;
    }
});
app.component('variantForm', {
    templateUrl: variant_form_template_url,
    controller: function(HelperService, $rootScope, $routeParams, $scope, $http, $location) {
        ...
        $http.get(
            laravel_routes['getVariantFormDetails'], {
                params: {
            ...
            self.variant = response.data.variant_details;
        });
        ...
        var v = jQuery(form_ids).validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                        url: laravel_routes['saveVariant'],
                        method: "POST",
                        data: formData,
                        processData: false,
                        contentType: false,
                    ...
                    .done(function(res) {
                        if (!res.success) {
                            ...
                            $('#submit').button('reset');
                            $location.path('/part-pkg/variant/list')
                            $scope.$apply()
                        }
                    })
                    .fail(function(xhr) {
...
app.component('variantView', {
    templateUrl: variant_view_template_url,
    controller: function(HelperService, $rootScope, $routeParams, $scope, $http, $location) {
        var id = $routeParams.id;
        var self = this;
        self.hasPermission = HelperService.hasPermission;
